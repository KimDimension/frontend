  ...
  return (
    <main style={{ padding: '28px 32px', minHeight: '100vh' }}>
      ...
      <div style={{ marginBottom: 22 }}>
        <h1 style={{ margin: 0, fontSize: 22, fontWeight: 900, color: C.text, letterSpacing: '-0.04em' }}>환자 관리</h1>
        <div style={{ fontSize: 13, color: C.textMuted, marginTop: 3 }}>전체 담당 환자 목록 — 날짜 무관</div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 22 }}>
        {[
          ...
          <div key={c.label} style={{ background: '#fff', borderRadius: 12, border: `1px solid ${C.border}`, padding: '16px 20px', boxShadow: '0 1px 4px rgba(0,0,0,0.05)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 6 }}>
              <span style={{ fontSize: 14 }}>{c.icon}</span>
              <span style={{ fontSize: 12, color: C.textMuted }}>{c.label}</span>
            </div>
            <div style={{ fontSize: 28, fontWeight: 800, color: c.color ?? C.text, letterSpacing: '-0.03em', lineHeight: 1 }}>{c.value}</div>
          </div>
        ))}
      </div>
      ...
      {error ? (
        ...
        <div style={{ background: '#fff', borderRadius: 14, border: `1px solid ${C.border}`, overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.05)' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', tableLayout: 'fixed' }}>
            <colgroup>
              <col style={{ width: '14%' }} />{/* 환자명 */}
              <col style={{ width: '8%'  }} />{/* 환자번호 */}
              <col style={{ width: '13%' }} />{/* 전화번호 */}
              <col style={{ width: '11%' }} />{/* 최근 위험도 */}
              <col style={{ width: '10%' }} />{/* 총 기록 수 */}
              <col style={{ width: '13%' }} />{/* 마지막 기록일 */}
              <col style={{ width: '13%' }} />{/* 경과 */}
            ...
            <tbody>
              {loading ? (
                ...
                <tr>
                  <td colSpan={8} style={{ padding: '48px 16px', textAlign: 'center', color: C.textMuted, fontSize: 13 }}>
                  ...
                  >
                    ...
                    <td style={{ padding: '13px 12px' }}>
                      {p.total_records > 0 ? (
                        <span style={{ fontSize: 13, fontWeight: 700, color: C.text }}>
                          {p.total_records}
                          <span style={{ fontSize: 11, color: C.textMuted, fontWeight: 400, marginLeft: 2 }}>건</span>
                        </span>
                      ) : (
                    ...
                    <td style={{ padding: '13px 12px' }} onClick={e => e.stopPropagation()}>
                      <button
                        onClick={() => navigate(`/doctor/patients/${p.id}`, { state: { patientName: p.name } })}
                        style={{
                          padding: '5px 12px', border: `1px solid ${C.border}`, borderRadius: 7,
                          background: isOverdue ? C.warningLight : C.primaryLight,
                          color: isOverdue ? C.warning : C.primaryDark,
